...
func physics_movement(delta: float, player_body: PlayerBody):
	...
	var wind_velocity := _active_wind_area.to_global(
			_active_wind_area.wind_vector * wind_multiplier
		) - _active_wind_area.global_transform.origin

	# Drag the player into the wind
	player_body.velocity = lerp(player_body.velocity, wind_velocity, drag * delta)
